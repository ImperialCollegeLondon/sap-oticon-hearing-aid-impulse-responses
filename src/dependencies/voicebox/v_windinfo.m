...
%      Version: $Id: v_windinfo.m 10871 2018-09-24 17:26:01Z dmb $
%
...
if ~nargout
    ...
    legend(['Equiv Noise BW = ' sprintsi(x.enbw,-2) '÷ Length'],['Max sidelobe = ' sprintf('%.0f',x.sidelobe) ' dB'],['-3 & -6dB BW = ' sprintsi(x.band3,-2) ' & ' sprintsi(x.band6,-2) '÷ Length']);
    hold off
    axis([0 ff(end) max(dd)-dbrange max(dd)+2]);
    ylabel('Gain (dB)');
    xlabel(sprintf('Freq×Length (%s×s)',xlab));
    %
    ...
    if length(x.cola)>4
        tcola=sprintf(',%d',x.cola(1:4));
        tcola=[tcola ',…'];
    else
        tcola=sprintf(',%d',x.cola);
    end
    if length(x.cola2)>4
        tcola2=sprintf(',%d',x.cola2(1:4));
        tcola2=[tcola2 ',…'];
    else
